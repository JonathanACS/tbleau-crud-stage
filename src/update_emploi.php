...
if ($_POST) {
    if (isset($_POST["id_emploi"]) && !empty($_POST["id_emploi"])
        ...
        // Message à afficher 
        $_SESSION["message"] = "Information de la recherche d'emploi modifiée";
...
<head>
    ...
    <title>Modifier le tableau d'emploi</title>
    <link rel="stylesheet" href="./css/style.css" />
</head>

<body>
    <?php include_once("./include/nav.php");?>
    <h1 class="title-center">Modifier une information du tableau de recherche d'entreprise</h1>
    <form method="post">
        <div class="form">
            <label for="entreprise">Entreprise</label>
            <input type="text" id="entreprise" name="entreprise"
                value="<?= isset($result["entreprise"]) ? htmlspecialchars($result["entreprise"]) : '' ?>">
        </div>
        <div class="form">
            <label for="statut">Statut</label>
            <input type="text" id="statut" name="statut"
                value="<?= isset($result["statut"]) ? htmlspecialchars($result["statut"]) : '' ?>">
        </div>
        <div class="form">
            <label for="dates">Date</label>
            <input type="date" id="dates" name="dates"
                value="<?= isset($result["dates"]) ? htmlspecialchars($result["dates"]) : '' ?>">
        </div>
        <div class="form">
            <label for="relance">Relance</label>
            <input type="date" id="relance" name="relance"
                value="<?= isset($result["relance"]) ? htmlspecialchars($result["relance"]) : '' ?>">
        </div>
        <div class="form">
            <label for="website">Website</label>
            <input type="text" id="website" name="website"
                value="<?= isset($result["website"]) ? htmlspecialchars($result["website"]) : '' ?>">
        </div>
        <div class="form">
